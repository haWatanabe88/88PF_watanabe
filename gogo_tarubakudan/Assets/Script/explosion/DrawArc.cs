...
public class DrawArc : MonoBehaviour
{
    ...
    public void setDrawArc(bool val)
    {
        drawArc = val;
    }
    ...
    private void Awake()
    {
        mask = ~LayerMask.GetMask("CameraCollision");
    }
    ...
    void Start()
    {
        ...
        pointerObject.transform.rotation = Quaternion.Euler(90f, 0f, 0f);//マーカーを生成した後に、自前で回転させている
    ...
    private float GetArcHitTime(float startTime, float endTime)
    {
        ...
        // 衝突判定
        RaycastHit hitInfo;
        if (Physics.Linecast(startPosition, endPosition, out hitInfo, mask))
        {
